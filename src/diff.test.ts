...
describe('diff', () => {
    ...
    test('global metadata', () => {
        expect(
            diff(
                {} as StyleSpecification,
                {
                    metadata: {'mapvina:author': 'nobody'}
                } as StyleSpecification
            )
        ).toEqual([]);
    });
    ...
    test('layer metadata', () => {
        expect(
            diff(
                {
                    layers: [{id: 'a', metadata: {'mapvina:group': 'Group Name'}}]
                } as StyleSpecification,
                {
                    layers: [{id: 'a', metadata: {'mapvina:group': 'Another Name'}}]
                } as StyleSpecification
            )
        ).toEqual([]);
    });
    ...
    test('add terrain', () => {
        ...
        ).toEqual([{command: 'setTerrain', args: [{source: 'mapvina-dem', exaggeration: 1.5}]}]);
    });
